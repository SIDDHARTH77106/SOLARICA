...
const Footer = () => {
  ...
  // 2. Animations
  // "SOLARICA" text thoda slow move karega scroll ke sath (Parallax)
  const yText = useTransform(scrollYProgress, [0, 1], [-100, 0]);
  const opacityText = useTransform(scrollYProgress, [0.5, 1], [0.2, 1]);

  return (
    // Wrapper div for height spacing (taaki footer overlap na kare content ko)
    <div 
      ref={footerRef} 
    ...
    >
      ...
      {/* --- STICKY FIXED FOOTER --- 
         Ye footer screen ke bottom par chipka rahega, 
         aur upar wala content iske upar se scroll hokar jayega (Uncover Effect)
      */}
      <div className="fixed bottom-0 left-0 w-full h-screen bg-[#0f1c15] text-[#e8e4dc] flex flex-col justify-end">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center overflow-hidden pointer-events-none z-0">
            <motion.h1 
                ...
                className="text-[15vw] leading-none font-bold tracking-tighter text-white/3 select-none whitespace-nowrap"
            >
                SOLARICA ENERGY
            </motion.h1>
        </div>
        ...
        <div className="container mx-auto px-6 relative z-10 pb-10 pt-20">
            ...
            <div>
               ...
               <div className="flex flex-col md:flex-row gap-12 text-sm tracking-widest uppercase text-gray-400">
                  ...
                  <div>
                    ...
                    <a href="mailto:Kiran@solarica.in" className="text-white hover:text-orange-500 transition cursor-pointer">siddharth@solarica.in</a>
                  </div>
               </div>
            </div>
          ...
          {/* Bottom Bar */}
          <div className="flex justify-between items-center text-xs text-gray-600 uppercase tracking-widest border-t border-white/5 pt-8">
            <p>© 2025 Solarica Energy India</p>
